  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <ResultCard title="canSend(from, 0, amount)" state={canSend} />
        <ResultCard title="canReceive(to, 0, amount)" state={canReceive} />
        <ResultCard title="canTransfer(from, to, 0, amount)" state={canTransfer} />
        <ResultCard
          title="getFrozenTokens(from, 0)"
          state={frozenFrom}
...
function ResultCard<T>({
  ...
  return (
    <div className="rounded-lg border border-zinc-200 p-3 dark:border-zinc-800">
      <p className="text-xs font-mono text-zinc-500">{title}</p>
      <p className={`mt-1 truncate text-sm font-semibold ${tone}`}>{body}</p>
    </div>
  );
}
